...
interface KeycloakPayloadWithRoles {
  realm_access?: {
    roles?: string[];
  };
...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const req = context.switchToHttp().getRequest<{ user?: KeycloakPayloadWithRoles }>();
    const tokenRoles = this.getTokenRoles(req.user);
    const hasRequiredRole = requiredRoles.some((role) => tokenRoles.has(role.toLowerCase()));
  ...
  private getTokenRoles(user: KeycloakPayloadWithRoles | undefined): Set<string> {
